...
- **Granularity and volume:** 11,703 boreholes, 122,693 stratigraphic intervals, and 239,137 SPT observations. The three Parquet assets total 11,993,633 bytes. The FL snapshot drops 364 `pdf_vlm_empty` boreholes because they contain no usable stratigraphy and have no related layer or SPT rows.
...
- **Provenance:** 6,462 boreholes are parsed from source JGS XML and 5,241 are reconstructed from column-diagram PDFs by a VLM. `data_source` must remain attached to every record; VLM-extracted fields are not source observations. The processing report's 12,067-row total is the pre-filter run result, not the current FL count.
- **Known gaps:** elevation is absent for 86 boreholes and groundwater is populated for 6,030. Survey purpose, precision, units in legacy tests, elevation datum, and freshness vary. SPT values above 100 require review; the reported maximum of 794 is an apparent source outlier.
