...
frontend は `data/` を直接読みません。file location、cache、filter、aggregate、rank は Python にあります。[呼び出し時系列](architecture/README.ja.md)を参照してください。
...
- [概念 architecture](docs/architecture/FL_SL_AL_CONCEPT.ja.md)
- [ADR-0001](docs/adr/0001-fl-sl-al-conceptual-layers.ja.md)
- [refactor 記録](docs/refactor/2026-07-fl-sl-al-factor-of-concept.ja.md)
- [frontend–backend 呼び出し時系列](architecture/README.ja.md)
...
- 完全な用途・費用・license：[`DATA_SOURCES.ja.md`](DATA_SOURCES.ja.md)。
...
出力は `data/mobara/tepco_grid_screen.json`。自動取得は権利条件を変えず、地域発見/事前相談順位専用で正式接続検討ではありません。CSV geometry は各 cell への容量配賦に不足します。`data/external/tepco/README.ja.md` を参照。
...
購入不要の確認済候補は ESA WorldCover、Sentinel-2 L2A、GSI Hazard open layer、MAFF筆、登記地図、MLIT不動産API、国土/環境GIS、METI FIT/FIP です。Dynamic World は評価後除外記録だけに残します。詳細：[`DATA_SOURCES.ja.md`](DATA_SOURCES.ja.md)。
