...
- 空間整合は接続の前提条件です。GIS data を持つか、座標・標高（明示された鉛直基準）・投影で整合できる source だけが FL に入ります。整合手段のない source を地理参照を捏造して描くことはなく、source 台帳に理由付きで「未接続」と記録します。

現在の GSI、MLIT の土地・災害・交通・地価レイヤ、OSM、横浜 open data、NASA POWER、TEPCO 公開 CSV、Google Satellite Embedding、および user 提供の KuniJiban 地下抽出は FL です。Embedding は foundation model 由来でも外部生成 FL 表現です。KuniJiban の各 row は source XML 由来か VLM 抽出かを保持し、model 抽出 field を source 観測として再表示しません。大容量 layer は frontend 起動 payload に含めず、catalog 登録してオンデマンド照会します。商用利用禁止 source は接続対象から除外し、評価資料だけを残す場合があります。
