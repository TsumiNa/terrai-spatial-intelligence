...
出力は `data/mobara/tepco_grid_screen.json`。自動取得は権利条件を変えず、地域発見/事前相談順位専用で正式接続検討ではありません。CSV geometry は各 cell への容量配賦に不足します。[`tepco-grid`](../../data/tepco-grid/README.ja.md) を参照。
...
購入不要の確認済候補は ESA WorldCover、Sentinel-2 L2A、GSI Hazard open layer、MAFF筆、登記地図、MLIT不動産API、国土/環境GIS、METI FIT/FIP です。Dynamic World は評価後除外記録だけに残します。詳細：[`open-data-landscape`](../open-data-landscape/README.ja.md)。
