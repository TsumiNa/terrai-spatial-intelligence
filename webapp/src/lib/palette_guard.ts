/** Rejects colors written outside the palette.
 *
 *  The lock is on the palette, not the geometry. A color outside the palette
 *  fragments the brand, is where a model most plausibly invents a near miss,
 *  and carries a contrast risk. A radius or a spacing step is a judgement with
 *  no correctness dimension, so `p-[13px]` and `rounded-[7px]` are legitimate
 *  and this must not touch them.
 *
 *  Two more things it must not touch, both of which a naive rule would break:
 *
 *  - **Arbitrary variants.** `data-[state=open]:` and `aria-[expanded=true]:`
 *    select a state rather than carry a value, and the dialog primitive coming
 *    in stage 05 requires them. A variant is everything before the final `:`;
 *    the rule reads only what follows.
 *  - **Interpolated values.** `style={`background:${item.color}`}` passes a
 *    color that came from the palette through a data path. The literal is what
 *    matters, not the property.
 */

/** Utilities whose arbitrary value is a color rather than a measurement. */
const COLOR_UTILITIES = [
  "bg",
  "text",
  "border",
  "outline",
  "ring",
  "fill",
  "stroke",
  "decoration",
  "divide",
  "accent",
  "caret",
  "shadow",
  "from",
  "via",
  "to",
];

/** A hand-written color, as opposed to a reference to one.
 *
 *  The function forms must be followed by a digit: `rgba(palette.red, 0.2)` is
 *  a call passing a palette entry, while `rgba(215, 91, 76, 1)` is a literal.
 *  Matching the bare function name flagged every use of this project's own
 *  `rgba` helper, which is how colors reach deck.gl.
 */
const COLOR_LITERAL = /#[0-9a-fA-F]{3,8}\b|\b(?:rgba?|hsla?|oklch|lab)\(\s*[\d.]/;

const CLASS_ATTRIBUTE = /class(?:Name)?\s*=\s*(?:"([^"]*)"|'([^']*)'|\{`([^`]*)`\})/g;
const STYLE_BLOCK = /<style[^>]*>([\s\S]*?)<\/style>/g;

export type Violation = { file: string; value: string; reason: string };

/** A color named in a comment is not a color being used, and this file's own
 *  patterns would otherwise trip it. */
function withoutComments(source: string): string {
  return source
    .replace(/\/\*[\s\S]*?\*\//g, "")
    // Trailing comments count too, but `https://` must survive: only strip a
    // `//` that is not preceded by a colon.
    .replace(/(?<!:)\/\/.*$/gm, "");
}

/** The utility is what follows the final `:`; everything before it is a variant. */
function utility(className: string): string {
  const parts = className.split(":");
  return parts[parts.length - 1] ?? className;
}

function arbitraryColorUtilities(source: string): string[] {
  const found: string[] = [];
  for (const match of source.matchAll(CLASS_ATTRIBUTE)) {
    const value = match[1] ?? match[2] ?? match[3] ?? "";
    for (const className of value.split(/\s+/).filter(Boolean)) {
      const util = utility(className);
      const bracket = util.indexOf("-[");
      if (bracket < 0) continue;
      const prefix = util.slice(0, bracket).replace(/^-/, "");
      const inner = util.slice(bracket + 2, util.lastIndexOf("]"));
      if (!COLOR_UTILITIES.includes(prefix)) continue;
      // `shadow-[0_2px_4px]` is geometry; `shadow-[#000]` is color.
      if (!COLOR_LITERAL.test(inner)) continue;
      found.push(className);
    }
  }
  return found;
}

function literalsInStyleBlocks(source: string): string[] {
  const found: string[] = [];
  for (const block of source.matchAll(STYLE_BLOCK)) {
    for (const line of withoutComments(block[1] ?? "").split("\n")) {
      // An interpolated value carries a palette color through a data path.
      if (line.includes("${") || line.includes("var(--")) continue;
      const match = line.match(COLOR_LITERAL);
      if (match) found.push(line.trim());
    }
  }
  return found;
}

/** Color literals are declared in one file; everywhere else imports from it. */
export const PALETTE_SOURCE = "src/lib/theme.ts";

/** The stylesheet inherited from the exhibition as built.
 *
 *  It carries dozens of colors picked by eye before any of this existed, so it
 *  is inventoried as an exact set rather than cleaned. Every *other* stylesheet
 *  is held to the same rule as components: no literal outside the theme block.
 */
export const INVENTORIED_STYLESHEET = "src/app.css";

/** Color literals written in a stylesheet, outside its `@theme` block.
 *
 *  `app.css` holds the styling the exhibition was built with, and it contains
 *  36 one-off tints picked by eye. They are not worth turning into palette
 *  entries — that would make the palette a color dump rather than a design
 *  system — and they disappear with the redesign. So they are inventoried
 *  rather than fixed, and the test asserts the set exactly: a 37th fails.
 */
export function stylesheetLiterals(source: string): string[] {
  const themeStart = source.indexOf("@theme");
  const body =
    themeStart < 0 ? source : source.slice(0, themeStart) + source.slice(source.indexOf("}", themeStart));
  const found: string[] = [];
  // Strip the var() references rather than skipping the line that holds them.
  // These rules are written one per line, so a line very often carries both —
  // `linear-gradient(145deg, var(--lime), #50aa75)` — and skipping it hid every
  // literal that shared a line with a token reference.
  for (const line of withoutComments(body).replace(/var\(--[a-z-]+\)/g, "").split("\n")) {
    for (const match of line.matchAll(/#[0-9a-fA-F]{3,8}\b|rgba?\(\s*[\d.][^)]*\)/g)) {
      found.push(match[0].toLowerCase());
    }
  }
  return found;
}

export function inspect(file: string, source: string): Violation[] {
  const violations: Violation[] = [];

  for (const value of arbitraryColorUtilities(source)) {
    violations.push({ file, value, reason: "arbitrary color utility" });
  }

  if (file.endsWith(".svelte")) {
    for (const value of literalsInStyleBlocks(source)) {
      violations.push({ file, value, reason: "color literal in a style block" });
    }
  }

  if (file.endsWith(".css") && !file.endsWith(INVENTORIED_STYLESHEET)) {
    for (const value of stylesheetLiterals(source)) {
      violations.push({ file, value, reason: "color literal outside the theme block" });
    }
  }

  if (file.endsWith(".ts") && !file.endsWith(PALETTE_SOURCE) && !file.endsWith("_test.ts")) {
    for (const line of withoutComments(source).split("\n")) {
      if (!COLOR_LITERAL.test(line)) continue;
      violations.push({ file, value: line.trim(), reason: `color literal outside ${PALETTE_SOURCE}` });
    }
  }

  return violations;
}
