...
- State: **Completed** — PR1 (drop slope basemap + 2.5D toggle + per-mode terrain, #85), PR2 (hillshade colour-by-height tint, #86), and PR3 (DEM-computed hillshade via a DEM1A→DEM5A→DEM10B resolution chain + optional 2× DPR) done.
- Note: The concrete form of the "setTerrain is orthogonal to the basemap" insight from the tile design. Removing the slope **basemap** does not touch the building slope-risk **analysis** (`interactive-al-compute`). PR3 corrected an earlier physics error: hillshade z16-17 sharpness needs **1 m DEM1A (≈z17)**, not 5 m DEM5A (which caps at z15) — and GSI serves `dem1a_png` **live** (LiDAR coverage), so **no self-hosting and no `osm-basemap-tiles` dependency**; the `gsidem://` protocol resolves a per-tile chain (DEM1A→DEM5A→DEM10B) with elevation-correct overscale. Delivered after the MapLibre v6 upgrade and before `osm-basemap-tiles` PR4 (extrusion builds on the final toggle/terrain model).
...
- State: **Completed** — the full PR sequence merged (#88 through #94, the acquisition split across #88/#90 with a #89 download-URL fix) under the owner's roadmap directive; one self-hosted, offline-capable merged building tileset now backs the basemap. PR1 (`fgd_kanto` acquisition mechanism, hand-parsing pinned FGD JPGIS(GML) 建築物の外周線 into `buildings.geojson`, each feature `footprint_source: "fgd"` + FGD id) merged as #88, with a download-URL fix in #89. The owner then completed the registration-gated download (4 prefectures, 2026-04-30, 209 2次メッシュ, 2.3 GB). A follow-up clips it to **mainland Kanto** (dropping 29 Pacific-island meshes 東京都 legally owns — Izu/Ogasawara, Minamitorishima, Okinotorishima), emitting a committed `coverage.json` footprint (180 meshes, bbox ≈ (138.63, 34.83, 140.88, 36.33)) that redefines the Kanto window from the real data. PR2 (merged tile generation, #91) merged **13,640,510 buildings** (5.18M OSM primary + 8.46M FGD fill; 4.85M FGD dropped where OSM covered the ground, no double-draw) into a **1.1 GB PMTiles z0–16** via tippecanoe, OSM clipped to the coverage footprint. PR3 (basemap integration) puts the merged PMTiles on the map as a MapLibre vector fill below the z16 handover, replacing GSI's building texture inside the coverage footprint (configurable tile URL via `?buildings=` for R2/minio/GCS), and adds the owner's **out-of-service boundary** — at building zoom, a viewport wholly outside coverage hides our tiles, falls back to GSI's buildings, and shows a trilingual badge. PR4 (PLATEAU height + 2.5D extrusion) distils PLATEAU LOD1 `measuredHeight` for the 125 modelled mainland-Kanto municipalities (CityGML streamed straight from the pinned CKAN zips), bakes `height`/`height_source` into the tiles by three tiers (PLATEAU measured → OSM `building:levels` → class estimate), and adds a `fill-extrusion` layer shown in the 2.5D view (PLATEAU joins the attribution). PR5 (retire the windowed path) makes the tile buildings clickable to a provenance popup built from the baked properties (no API call), removes the z16 handover/clamp (tiles span all zooms, re-baked to z17), and drops `osmBuildings` from the served datasets + store — one building source, clickable at every inspection zoom.
- Note: One merged tileset, not two overlaid layers — the OSM-primary / 基盤地図情報-fill decision is resolved at build time into a single building layer, each feature tagged `footprint_source` (`osm` | `fgd`); render never double-draws. Tiles carry **render/identity only** (`feature_id`, `footprint_source`, `building`, `height`); analytical materials (slope/relief/…) go in the FL store keyed by `feature_id`, not the tiles — the render/analyse boundary, precomputed by `interactive-al-compute`. Scope is buildings only, so it does **not** replace the live-GSI style JSON or the non-building vector layers (roads/water/labels/land) — hardening those is `basemap-resilience`, a complementary refactor. Reframed 2026-07-24 from OSM-only tiles to the OSM + 基盤地図情報 + PLATEAU merge after confirming OSM-only would read empty outside dense cities. Licence cleared: 基盤地図情報 is **測量法 承認申請-exempt** (attribution + 加工表示 only), PLATEAU is Site Policy §3 — both fine for offline, commercial, self-built distribution (see `docs/summary/government-3d-building-sources/`). Feasibility unchanged (3–15 min preprocessing, ~300–700 MB PMTiles, near-zero serving CPU); near-free on a zero-egress host (Cloudflare R2 + CDN), GCP egress-driven ~$5–320/mo. The owner's roadmap directive authorized execution; PR1 is underway, with PR2 gated on the registration-gated FGD download.
...
- State: **Completed** — PR1 (style + sprite snapshot, #82) and PR2 (production-raster fallback) done; a follow-up hotfix (#83) resolved the vendored sprite to an absolute URL that v6 requires.
