...
The frontend never reads `data/` directly. File locations, caching, filtering, aggregation, and ranking stay in Python. See [the call sequence diagrams](../../architecture/FRONTEND_BACKEND.md).
...
- [Concept architecture](../../architecture/FL_SL_AL_CONCEPT.md)
...
- [Frontend–backend call sequences](../../architecture/FRONTEND_BACKEND.md)
...
Output: `data/mobara/tepco_grid_screen.json`. Download automation does not change TEPCO rights. Values rank regional discovery/pre-consultation only, not formal connection studies, and CSV geometry cannot allocate capacity to each cell. See [`tepco-grid`](../../data/tepco-grid/README.md).
