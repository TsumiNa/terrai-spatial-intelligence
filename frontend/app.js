const API_ORIGIN = new URLSearchParams(window.location.search).get("api") || "http://127.0.0.1:8000";
const API_BASE = `${API_ORIGIN.replace(/\/$/, "")}/api/v1`;
const ASSET_BASE = `${API_BASE}/assets`;
...
const REGIONS = {
  yokohama: {
    ...
    tiles: {
      standard: `${ASSET_BASE}/tiles/yokohama/{z}/{x}-{y}.png`,
      photo: `${ASSET_BASE}/tiles/yokohama/photo/{z}/{x}-{y}.jpg`,
      hillshade: `${ASSET_BASE}/tiles/yokohama/hillshade/{z}/{x}-{y}.png`,
      slope: `${ASSET_BASE}/tiles/yokohama/slope/{z}/{x}-{y}.png`
    }
  },
  mobara: {
    label: "千叶 · 茂原市",
    ...
    tiles: {
      standard: `${ASSET_BASE}/tiles/mobara/{z}/{x}-{y}.png`,
      photo: `${ASSET_BASE}/tiles/mobara/photo/{z}/{x}-{y}.jpg`,
      hillshade: `${ASSET_BASE}/tiles/mobara/hillshade/{z}/{x}-{y}.png`,
      slope: `${ASSET_BASE}/tiles/mobara/slope/{z}/{x}-{y}.png`
    }
  }
};
...
function applyStaticTranslations() {
  ...
  if (state.data.meta) {
    document.getElementById("service-datasets").textContent = `${state.data.meta.datasets_ready} / ${state.data.meta.datasets_total}`;
    document.getElementById("service-sources").textContent = `${state.data.meta.source_groups}${state.lang === "en" ? " " : ""}${t("组")}`;
  }
}
...
function useRegion(regionKey) {
  ...
  document.getElementById("region-pill").textContent = t(region.label);
}
...
function setHeader(eyebrow, title, kicker, heroTitle, description, formula = "") {
  ...
  setEvidenceStatus([["observed", "数据已核查"], ["proxy", "筛查结果可追溯"]]);
}
...
function setQueueHeading(eyebrow, title, count) {
  ...
  const explanations = {
    overview: state.view === "renewable" ? "优先显示工程可达、坡度较低且靠近输电背景的单元。" : "优先显示兼顾屋顶能源潜力、道路可达性与周边需求的候选点。",
    evidence: "变化分越高，越值得结合影像、业务记录和现场进行复核。",
    slope: "分数越高，越应优先核查边坡、排水与建筑现场状况。",
    roads: "分数越高，越可能同时影响道路连续性与周边高暴露建筑。",
    facilities: "排序综合场址安全、道路可达、周边需求与屋顶能源潜力。",
    solar: "优选单元适合进入地权、许可、生态与并网的下一轮尽调。",
    joint: state.view === "corridors" ? "把道路与沿线建筑风险合并为可成批实施的巡检走廊。" : "寻找能够同时服务社区需求与分布式能源部署的候选节点。",
    development: state.view === "constraints" ? "排除项用于解释当前冲突，不代表法律上的绝对不可开发。" : "推荐单元适合优先开展现场踏勘与并网预咨询。"
  };
  document.getElementById("queue-explanation").textContent = t(explanations[state.module] || "");
}
...
function imageOverlay(path, bounds, opacity = .78) {
  const assetPath = path.startsWith("data/") ? `${ASSET_BASE}/${path.slice(5)}` : path;
  const layer = L.imageOverlay(assetPath, bounds, { opacity, interactive: false }).addTo(map);
  state.vectorLayers.push(layer);
  return layer;
}
...
function renderOverview() {
  ...
  setHeader(
    "项目组合", "城市韧性与新能源机会", "PORTFOLIO OPPORTUNITY",
    "一眼找到最值得优先核查的项目",
    "横滨展示城市韧性投资机会，茂原展示地面光伏开发机会；每个候选结果都能回到原始来源、计算逻辑和适用限制。",
    "<span>项目发现</span><i>→</i><span>证据核查</span><i>→</i><strong>现场尽调</strong>"
  );
  setMetrics([
  ...
  if (isUrban) {
    ...
    const highBuildings = state.data.recommendations.slope.features;
    ...
    const items = state.data.recommendations.hubs.features;
    ...
    const items = state.data.recommendations.delivery.features;
...
function renderEvidence() {
  ...
  const evidence = region === "yokohama"
    ? state.data.recommendations.embeddingYokohama.features
    : state.data.recommendations.embeddingMobara.features;

  setHeader(
    "证据与可靠性", "验证每个结果背后的数据", "EVIDENCE & RELIABILITY",
    mode === "change" ? "快速发现年度变化，再结合影像与现场判断原因" : "寻找空间相似区域，为后续核查提供线索",
    "10 m 遥感证据与建筑、道路和项目单元保持独立血缘；点击任何虚线数值即可查看来源、版本、公式与限制。",
    "<span>来源</span><i>→</i><span>计算</span><i>→</i><span>限制</span><i>→</i><strong>可追溯结论</strong>"
  );
  setEvidenceStatus([
  ...
  document.getElementById("method-card").innerHTML = `<strong>如何使用这项证据</strong><br>年度变化用于提示“哪里值得复核”，相似表征用于寻找“哪里可能具有相近背景”。二者都不直接等同土地类别、风险或投资结论。`;
...
function renderFacilities() {
  ...
  state.view = view;
  useRegion("yokohama");
  const facilities = state.data.recommendations.facilities.features;
  const pv = state.data.facilitySummary.pv_kwp_proxy;
  const demandLinks = state.data.facilitySummary.served_high_risk_buildings;
  const meanScore = state.data.facilitySummary.mean_resilience_score;
  setHeader(
    "横滨 / 公共设施改造", "真实防灾设施的韧性升级机会", "PUBLIC FACILITY RESILIENCE",
    "哪些公共设施最值得优先评估光储与通行改善",
    "结合横滨市官方防灾据点、屋顶容量代理、道路可达性和周边高暴露需求，形成现场核查队列。",
  ...
  setMetrics([
    ...
    metric("屋顶容量代理", pv, "kWp", "最近建筑轮廓 × 0.12", colors.green),
  ...
  document.getElementById("method-card").innerHTML = `<strong>为什么推荐这些设施</strong><br>设施身份和坐标来自横滨市官方数据；排序综合场址安全、道路可达、周边需求和屋顶能源潜力。点击分数可查看完整公式与代入值。`;
...
function renderSlope() {
  ...
  const all = state.data.buildings.features;
  const queueItems = state.data.recommendations.slope.features;
  const features = state.view === "high" ? queueItems : all;
...
function renderRoads() {
  ...
  const all = state.data.roads.features;
  const queueItems = state.data.recommendations.roads.features;
  const features = state.view === "high" ? queueItems : all;
  const ref = layerForFeatures(features, {
    style: feature => {
...
function renderSolar() {
  ...
  context.layer.bringToBack();
  const all = state.data.solar.features;
  const queueItems = state.data.recommendations.solar.features;
  const features = state.view === "preferred" ? queueItems : all;
  const ref = layerForFeatures(features, {
    style: feature => {
...
function renderJoint() {
  ...
  if (view === "hubs") {
    ...
    const high = state.data.recommendations.slope.features;
    ...
    features = state.data.recommendations.hubs.features;
    ...
    features = state.data.recommendations.corridors.features;
...
function renderDevelopment() {
  ...
  if (view === "delivery") {
    legend([["高交付性", colors.blue], ["候选", colors.green], ["输电线", "#8e5eaa"]]);
    features = state.data.recommendations.delivery.features;
    ...
    features = state.data.recommendations.constraints.features;
...
fetchJson(`${API_BASE}/bootstrap`)
  .then(payload => {
    state.data = payload;
    document.getElementById("service-datasets").textContent = `${payload.meta.datasets_ready} / ${payload.meta.datasets_total}`;
    document.getElementById("service-sources").textContent = `${payload.meta.source_groups} ${t("组")}`;
