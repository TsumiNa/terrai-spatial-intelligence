(function exposeTerrAIAudit(global) {
  ...
  function field(label, value, props = {}) {
    ...
    if (["证据状态", "年度", "类型", "等级", "支持率", "未计入评分", "有效像素", "语义类别", "原因", "冲突数", "高风险建筑", "暴露建筑", "服务需求", "平均建筑风险"].includes(label)) return raw(label, value, "Local cached GeoJSON property", label, "current Demo snapshot", "data/**/*.geojson", ml("字段含义和来源随对象类型而异；完整许可见 docs/data/catalog/README.zh.md。", "フィールドの意味と出典は対象種別で異なります。ライセンス詳細は docs/data/catalog/README.ja.md を参照してください。", "Field meaning and lineage vary by object type; see docs/data/catalog/README.md for licensing."));
