...
- State: **In progress** — execution begun under the owner's roadmap directive. PR1 (`fgd_kanto` acquisition mechanism, hand-parsing pinned FGD JPGIS(GML) 建築物の外周線 into `buildings.geojson`, each feature `footprint_source: "fgd"` + FGD id) merged as #88, with a download-URL fix in #89. The owner then completed the registration-gated download (4 prefectures, 2026-04-30, 209 2次メッシュ, 2.3 GB). A follow-up clips it to **mainland Kanto** (dropping 29 Pacific-island meshes 東京都 legally owns — Izu/Ogasawara, Minamitorishima, Okinotorishima), emitting a committed `coverage.json` footprint (180 meshes, bbox ≈ (138.63, 34.83, 140.88, 36.33)) that redefines the Kanto window from the real data. PR2 (merged tile generation) implements + spikes the merge: **13,640,510 buildings** (5.18M OSM primary + 8.46M FGD fill; 4.85M FGD dropped where OSM covered the ground, no double-draw) into a **1.1 GB PMTiles z0–16** via tippecanoe, OSM clipped to the coverage footprint. Remaining: PR3 (basemap integration + out-of-service boundary with GSI fallback outside coverage) → PR4 (PLATEAU height + 2.5D extrusion) → PR5 (retire the windowed path).
