...
- State: **In progress** — execution begun under the owner's roadmap directive. PR1 adds the 基盤地図情報 acquisition mechanism: the `fgd_kanto` task hand-parses the pinned FGD JPGIS(GML) 建築物の外周線 into `data/fgd/kanto_buildings/buildings.geojson` (each feature `footprint_source: "fgd"` + FGD id), verified against a synthetic FGD-schema fixture. The actual FGD download is registration-gated (a documented manual owner step) and gates PR2's OSM+FGD merge. PR2–PR5 (merged tile generation → basemap integration → PLATEAU height + 2.5D extrusion → retire the windowed path) remain.
