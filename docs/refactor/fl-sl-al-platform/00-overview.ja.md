...
曖昧な「統一データ基盤」の継続や、各 application が独自補完 model を持つ方式ではなく FL → SL → AL を採用します。統一名称は単純ですが事実、代理、予測を混同し続けます。application 別 model は短期納品が速くても、顧客学習、校正、監査が個別案件に閉じ共有資産になりません。

採用構造では observed と synthetic を区別し、model portfolio、適用性、abstention の位置を明確にし、新 application が検証済 SL を再利用できます。代わりに一部地域を unknown のまま受け入れ、version、権限、model 選択、lineage を後続実装する必要があります。所有、法定許認可、正式系統接続、構造安全は model 補完しません。
...
詳細は `docs/architecture/FRONTEND_BACKEND.ja.md`。
