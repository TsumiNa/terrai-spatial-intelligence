...
渲染器无关的札幌 handoff 位于 `data/plateau/uc24_13_sapporo/scene_handoff.json`，它把 195 个要素的 GeoJSON 作为观测出入口拓扑，并保留独立的 OSM 快照时间、查询哈希、ODbL 许可和审计路径。它不会把 OSM 要素吸附到 PLATEAU 表面，不推断深度，也不会把缺失 access 标签解释为许可。handoff 通过 `data/scenes/underground/catalog.json` 被发现；两份文件均不创建额外的 OSM 数据集 key，也不进入 `/bootstrap`。
