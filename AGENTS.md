...
`frontend/` is plain browser JS with a vendored Leaflet in `frontend/vendor/` and has no build step. The next frontend in `webapp/` (Svelte 5 + Vite + TypeScript) has its own npm toolchain — commands in `CONTRIBUTING.md`; its tests are colocated `*_test.ts` files run by Vitest, plus Playwright under `webapp/e2e/`. `webapp/openapi.json` and `webapp/src/lib/api/schema.d.ts` are generated by `npm run generate:api`, never hand-edited; `terrai_spatial/api_test.py` fails when they drift from the FastAPI application.
...
A successor frontend is being stood up in `webapp/` per `docs/refactor/maplibre-migration/`; `frontend/` stays the served default until that migration's stage 05 retires it.
