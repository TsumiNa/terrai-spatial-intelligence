...
该阶段只完成 Factor of Concept，不定义 schema、数据库或调度。后续客户展览重构增加了最小只读 API，但不改变三层概念边界。完整定义见 [`FL_SL_AL_CONCEPT.md`](../architecture/FL_SL_AL_CONCEPT.md)，决策理由已归入 [`fl-sl-al-platform/00-overview.md`](../refactor/fl-sl-al-platform/00-overview.md)。
...
- 详细边界见 [`FRONTEND_BACKEND.md`](../architecture/FRONTEND_BACKEND.md)。
