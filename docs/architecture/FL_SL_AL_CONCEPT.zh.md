...
当前 Demo 中的 GSI、MLIT 土地/灾害/交通/地价图层、OpenStreetMap、横滨市开放数据、NASA POWER、东京电力公开 CSV 与 Google Satellite Embedding 均属于 FL。Satellite Embedding 是外部生产的观测表征，不因为它来自基础模型就自动成为 TerrAI 的 SL。大型 MLIT 图层只登记目录并按需查询，不进入前端启动包；禁止商业使用的来源不予接入，但可保留评估文档。
