...
現在の GSI、MLIT の土地・災害・交通・地価レイヤ、OSM、横浜 open data、NASA POWER、TEPCO 公開 CSV、Google Satellite Embedding は FL です。Embedding は foundation model 由来でも外部生成 FL 表現です。大容量 MLIT レイヤは frontend 起動 payload に含めず、catalog 登録してオンデマンド照会します。非商用 source は local-only とし、顧客 API の外に置きます。
