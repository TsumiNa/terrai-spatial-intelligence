...
- 空间对齐是准入前提：只有自带 GIS 数据、或能通过坐标、标高（含明示的垂直基准）、投影方式对齐的来源才能进入 FL。没有任何对齐手段的来源绝不靠捏造地理参考上图，而是在来源登记表中记录为未接入并注明原因。

当前 Demo 中的 GSI、MLIT 土地/灾害/交通/地价图层、OpenStreetMap、横滨市开放数据、NASA POWER、东京电力公开 CSV、Google Satellite Embedding，以及用户提供的 KuniJiban 地下数据均属于 FL。Satellite Embedding 是外部生产的观测表征，不因为它来自基础模型就自动成为 TerrAI 的 SL。KuniJiban 每条记录保留其来自源 XML 还是 VLM 提取的标记；模型提取字段不得重新标注成源观测。大型图层只登记目录并按需查询，不进入前端启动包；禁止商业使用的来源不予接入，但可保留评估文档。

#### 来源优先级与时间契约

- 当全国数据和地方数据覆盖同一领域时，TerrAI 以全国数据作为覆盖基线。地方数据用于独立校验匹配记录，并增加地方属性或明确标记的补充记录；不得无痕覆盖全国记录。
- 每个 FL 数据集都必须携带 `retrieved_at` 时间戳。若发布方提供发布日期、更新时间、生效时间、观测时间或覆盖周期，则另行保留为 `source_updated_at` 等来源时间元数据；若发布方未提供，则记录为带原因的未知值，不能编造日期。
- 融合记录须保留每个参与来源的 provenance 与时间戳。来源冲突应保持可见、进入复核；周期性更新应比较时间戳和内容差异，而不能把最后下载的数据自动视为权威结论。
