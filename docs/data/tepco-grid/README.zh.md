...
2. 执行 `uv run python -m terrai_spatial fetch grid` 或 `uv run python scripts/update_tepco_grid.py --force`。
...
执行 `uv run python -m terrai_spatial fetch grid` 主动更新；`--offline` 可使用提交的标准化摘要，或从已有本地缓存重建。
